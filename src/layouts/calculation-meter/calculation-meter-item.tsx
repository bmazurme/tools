...
import CalculationMeterModal from './calculation-meter-modal';
...
import { METER_TYPE_LABEL } from './field-config';

import style from './calculation-meter-column.module.css';

type FormPayload = { name: string };
interface ICalculationMeterItem {
  item: (ItemType);
  index: number;
}

const CalculationMeterItem = memo(({ item, index }: ICalculationMeterItem) => {
  ...
  const [, drop] = useDrop({
    ...
    async hover(_item: (ItemType & CalculationMeter), monitor) {
      if (!ref.current) {
        return;
      }
    ...
    <li ref={ref} className="item" style={{ opacity }}>
      ...
      >
        <ul className="fields">
          ...
          <Text variant="code-1" className={style.meterType}>
            {METER_TYPE_LABEL[item.calculationMeter?.meterType ?? 'vane']}
          </Text>
          <Text variant="code-1" className={style.diameter}>
            {item.calculationMeter?.diameter}
          </Text>
          <Text variant="code-1" className={style.flowRate}>
            {item.calculationMeter?.flowRate}
          </Text>
          <Text
            variant="code-1"
            className={style.resistance}
            title={item.calculationMeter?.resistanceManual ? 'Указано вручную' : 'По таблице'}
          >
            {item.calculationMeter?.resistanceManual ?? item.calculationMeter?.resistance}
          </Text>
          <Text variant="code-1" className={style.pressureLoss}>
            {item.calculationMeter?.pressureLoss}
          </Text>
        </ul>
      </Item>
      {isModalOpen
        && (
        <CalculationMeterModal
          item={item}
          open={isModalOpen}
...
export default CalculationMeterItem;

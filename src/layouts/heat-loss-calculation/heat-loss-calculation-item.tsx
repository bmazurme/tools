...
import HeatLossCalculationModal from './heat-loss-calculation-modal';
...
interface IHeatLossCalculationItem {
  item: (ItemType);
  index: number;
}

const HeatLossCalculationItem = memo(({ item, index }: IHeatLossCalculationItem) => {
  ...
  const [, drop] = useDrop({
    ...
    async hover(_item: (ItemType & HeatLossCalculation), monitor) {
      if (!ref.current) {
        return;
      }
    ...
    <li ref={ref} className="item" style={{ opacity }}>
      ...
      >
        <ul className="fields">
          ...
          <Text variant="code-1" className={style.length}>
            {item.heatLossCalculation?.length}
          </Text>
          <Text variant="code-1" className={style.innerPipeDiameter}>
            {item.heatLossCalculation?.innerPipeDiameter}
          </Text>
          <Text variant="code-1" className={style.outerPipeDiameter}>
            {item.heatLossCalculation?.outerPipeDiameter}
          </Text>
          <Text variant="code-1" className={style.thickness}>
            {item.heatLossCalculation?.thickness}
          </Text>
          <Text variant="code-1" className={style.tIn}>
            {item.heatLossCalculation?.tIn}
          </Text>
          <Text variant="code-1" className={style.tOut}>
            {item.heatLossCalculation?.tOut}
          </Text>
          <Text variant="code-1" className={style.tStart}>
            {item.heatLossCalculation?.tStart}
          </Text>
          <Text variant="code-1" className={style.tEnd}>
            {item.heatLossCalculation?.tEnd}
          </Text>
        </ul>
      </Item>
      {isModalOpen
        && (
        <HeatLossCalculationModal
          item={item}
          open={isModalOpen}
...
export default HeatLossCalculationItem;

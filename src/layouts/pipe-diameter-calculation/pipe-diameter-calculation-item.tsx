...
import PipeDiameterCalculationModal from './pipe-diameter-calculation-modal';
...
interface IPipeDiameterCalculationItem {
  item: (ItemType);
  index: number;
}

const PipeDiameterCalculationItem = memo(({ item, index }: IPipeDiameterCalculationItem) => {
  ...
  const [, drop] = useDrop({
    ...
    async hover(_item: (ItemType & PipeDiameterCalculation), monitor) {
      if (!ref.current) {
        return;
      }
      ...
      >
        <ul className="fields">
          ...
          <Text variant="code-1" className={style.flowRate}>
            {item.pipeDiameterCalculation?.flowRate}
          </Text>
          <Text variant="code-1" className={style.velocity}>
            {item.pipeDiameterCalculation?.velocity}
          </Text>
          <Text variant="code-1" className={style.diameter}>
            {item.pipeDiameterCalculation?.diameter}
          </Text>
        </ul>
      </Item>
      {isModalOpen
        && (
        <PipeDiameterCalculationModal
          item={item}
          open={isModalOpen}
...
export default PipeDiameterCalculationItem;

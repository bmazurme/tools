...
import ThrottlePlateModal from './throttle-plate-modal';
...
interface IThrottlePlateItem {
  item: (ItemType);
  index: number;
}

const ThrottlePlateItem = memo(({ item, index }: IThrottlePlateItem) => {
  ...
  const [, drop] = useDrop({
    ...
    async hover(_item: (ItemType & ThrottlePlate), monitor) {
      if (!ref.current) {
        return;
      }
      ...
      >
        <ul className="fields">
          ...
          <Text variant="code-1" className={style.flowRate}>
            {item.throttlePlate?.flowRate}
          </Text>
          <Text variant="code-1" className={style.excessHead}>
            {item.throttlePlate?.excessHead}
          </Text>
          <Text variant="code-1" className={style.diameter}>
            {item.throttlePlate?.diameter}
          </Text>
        </ul>
      </Item>
      {isModalOpen
...
export default ThrottlePlateItem;

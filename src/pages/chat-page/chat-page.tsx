...
import {
  ChatContainer, type ChatType, type TChatMessage,
...
export default function ChatPage() {
  ...
  const loadChatHistory = useCallback(async (sessionId: string) => {
    ...
    try {
      const historyResponse = await getChatHistory(sessionId).unwrap();
  ...
  const handleCreateChat = useCallback(async () => {
    ...
    try {
      const sessionResponse = await createSession(user!.id).unwrap();
  ...
  useEffect(() => {
    const loadChatList = async () => {
      ...
      if (user) {
        try {
          const chatList = await getChatList(user!.id).unwrap();
